...
from bosdyn.client.frame_helpers import ODOM_FRAME_NAME, BODY_FRAME_NAME, get_se2_a_tform_b
...
from helpers import relative_move_cmd

from crossing import relative_move
from fronting_trajectory import relative_move_traj
from PC_detection import remove_ground

LOGGER = logging.getLogger()

VELOCITY_BASE_SPEED = 0.5  # m/s
VELOCITY_BASE_ANGULAR = 0.3  # rad/sec
...
class WasdInterface(object):
	...
	def __init__(self, robot):
		...
		self._command_dictionary = {
			...
			ord('S'): self._run_experiment_submissive,
			ord('D'): self._run_experiment_dominant,
			ord('L'): self._run_experiment_traj_left,
			ord('R'): self._run_experiment_traj_right,
			#ord('y'): self._freeze,
			#ord('o'): self._return_to_origin,
			ord('p'): self._get_position,
			ord('o'): self._return_to_start,
			ord('g'): self._get_position_goal,
			ord('h'): self._return_to_goal
		}
	...
	def _drive_draw(self, stdscr, lease_keep_alive):
		...
		stdscr.addstr(12, 0, "          [I]: Take image, [O]: Video mode, [j]: Arm stow")
		stdscr.addstr(13, 0, "          [f]: Stand, [r]: Self-right               ")
		stdscr.addstr(14, 0, "          [v]: Sit, [b]: Battery-change             ")
		stdscr.addstr(15, 0, "          [w]: Forward, [s]: Backward, [a]: Left, [d]: Right ")
		stdscr.addstr(16, 0, "          [q]: Turn left, [e]: Turn right             ")
		stdscr.addstr(17, 0, "          [l]: Return/Acquire lease,  [ESC]: Stop               ")
		stdscr.addstr(18, 0, "          [o]: Return to start position, [p]: Record start position")
		stdscr.addstr(19, 0, "          [h]: Return to goal position, [g]: Record goal position")
		stdscr.addstr(20, 0, "          [S]: Run experiment submissive, [D]:Run experiment dominant")
		stdscr.addstr(21, 0, "          [L]: Run experiment with trajectory left, [R]: Run experiment with trajectory right")
		stdscr.addstr(22, 0, "")
	...
	def _start_robot_command(self, desc, command_proto, end_time_secs=None):
		...
		def _start_command():
			cmd_id= self._robot_command_client.robot_command(command=command_proto,
	...
	def _stand(self):
		...
		self._start_robot_command('stand', RobotCommandBuilder.synchro_stand_command(footprint_R_body= footprint_R_body))
	...
	def _freeze(self):
		self._start_robot_command(
		'freeze',RobotCommandBuilder.freeze_command())	
	
	def _get_position(self):
		robot_state_client = self._robot_state_client
		state= robot_state_client.get_robot_state()
		#posi = state.kinematic_state.transforms_snapshot
		#print("position", posi)
		#time.sleep(2)
		transforms = robot_state_client.get_robot_state().kinematic_state.transforms_snapshot
		#self.add_message(str(transforms)) # This does not work
		self.add_message("Transform recorded")
		self.transforms_start = transforms
	
	def _get_position_goal(self):
		robot_state_client = self._robot_state_client
		state= robot_state_client.get_robot_state()
		#posi = state.kinematic_state.transforms_snapshot
		#print("position", posi)
		...
		transforms = robot_state_client.get_robot_state().kinematic_state.transforms_snapshot
		self.add_message("Transform recorded")
		self.transforms_goal = transforms

	def _return_to_start(self):
		body_tform_goal = math_helpers.SE2Pose(x=0, y=0, angle=0)

		out_tform_body = get_se2_a_tform_b(self.transforms_start, ODOM_FRAME_NAME, BODY_FRAME_NAME)
		out_tform_goal = out_tform_body * body_tform_goal
		self._start_robot_command(
			'fwd_and_rotate',
			RobotCommandBuilder.synchro_se2_trajectory_point_command(
				goal_x=out_tform_goal.x, goal_y=out_tform_goal.y, goal_heading=out_tform_goal.angle, frame_name=ODOM_FRAME_NAME, params=None,
				body_height=0.0, locomotion_hint=spot_command_pb2.HINT_TROT),
			end_time_secs=time.time() + 20)

	def _return_to_goal(self):
		body_tform_goal = math_helpers.SE2Pose(x=0, y=0, angle=0)

		out_tform_body = get_se2_a_tform_b(self.transforms_goal, ODOM_FRAME_NAME, BODY_FRAME_NAME)
		out_tform_goal = out_tform_body * body_tform_goal
		self._start_robot_command(
			'fwd_and_rotate',
			RobotCommandBuilder.synchro_se2_trajectory_point_command(
	...
	def _run_experiment_submissive(self):
		_, cmd_id, _ = relative_move('s', 6, 0, 0, ODOM_FRAME_NAME,
				self._robot_command_client, self._robot_state_client)
		...
		while running:
			...
			running = self.flush_and_estop_buffer(self.stdscr)

			time.sleep(0.5)

	def _run_experiment_dominant(self):
		_, cmd_id, _ = relative_move('d', 6, 0, 0, ODOM_FRAME_NAME,
				self._robot_command_client, self._robot_state_client)
	...
	def _run_experiment_traj_left(self):
		robot_cmd, cmd_id, start_time = relative_move_traj('n', 6, 0, 0, ODOM_FRAME_NAME, 'l',
						self._robot_command_client, self._robot_state_client, self._robot_image_client, False)
		for i in range(2):
			curses.beep()
			time.sleep(0.25)
		sources = ['frontleft_depth', 'frontright_depth']
    
		"Temporally setting variables here"
		mode ='n'
		body_height = 0
		avoiding_direction = 'l'
		...
		while running:
			...
			image_responses = self._robot_image_client.get_image_from_sources(sources)
				...
				if len(PC_center)> 20:
					if not_avoided:
						...
						if avoiding_direction == 'l':
							_, cmd_id, _ = relative_move(mode, 2.1, 0.7, 0.0, frame_name, self._robot_command_client, self._robot_state_client, self._robot_image_client)
						if avoiding_direction == 'r':
							_, cmd_id, _ = relative_move(mode, 2.1, -0.7, -0.0, frame_name, self._robot_command_client, self._robot_state_client, self._robot_image_client)

						feedback = self._robot_command_client.robot_command_feedback(cmd_id)
						mobility_feedback = feedback.feedback.synchronized_feedback.mobility_command_feedback
						# Scraped this because it tries to wait until the robot reaches the desired point.
						...
						# 		break
							...
							# avoiding = self.flush_and_estop_buffer(self.stdscr)

							# time.sleep(0.3)

						not_avoided = False
						# Wait for 3 seconds before getting back to the original trajectory (Note: esc key does not work during 3 seconds)
						time.sleep(3)
	...
	def _run_experiment_traj_right(self):
		relative_move_traj('n', 6, 0, 0, ODOM_FRAME_NAME, 'r',
						self._robot_command_client, self._robot_state_client, self._robot_image_client, False)
